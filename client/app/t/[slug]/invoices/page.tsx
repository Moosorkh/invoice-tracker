...
export default function Invoices() {
  ...
  const fetchInvoices = async () => {
    ...
    setError("");
    try {
      const res = await fetch(`/t/${slug}/api/invoices`, {
        headers: {
  ...
  const handleSubmit = async () => {
    setError("");
    try {
      const response = await fetch(`/t/${slug}/api/invoices`, {
        method: "POST",
        headers: {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await fetch(`/t/${slug}/api/invoices/${currentInvoice.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
    ...
    <Container>
      ...
      <Box sx={{ mb: 2, display: "flex", gap: 2, alignItems: "center" }}>
        ...
        <Button
          variant="outlined"
          color="primary"
          onClick={() => router.push(`/t/${slug}/clients`)}
        >
          Manage Clients
        </Button>
